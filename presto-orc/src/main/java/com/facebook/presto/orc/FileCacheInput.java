        ...
        extends FixedLengthSliceInput
{
    ...
    private FixedLengthSliceInput getDelegate(int length)
    {
        ...
        try {
            entry = FileCache.get(dataSource, loadOffset + bufferOffset, bufferBytes, listener, 100, Math.min(bufferBytes, viewLength), numMerged);
        }
        catch (IOException e) {
    ...
    private void loadPosition(int position)
    {
        ...
        try {
            entry = FileCache.get(dataSource, loadOffset + loadStart, bufferBytes, listener, 100, Math.min(viewLength, bufferBytes), numMerged);
        }
        catch (IOException e) {
            throw new UncheckedIOException(e);
        }
