        ...
        extends NullWrappingColumnReader
{
            ...
            implements LongInputStream.ResultsConsumer
    {
        ...
        private boolean test(int dictionaryId)
        {
            if (!deterministicFilter) {
                return evaluateFilter(dictionaryId);
            }

            if (filterResults[dictionaryId] == FILTER_NOT_EVALUATED) {
                filterResults[dictionaryId] = evaluateFilter(dictionaryId) ? FILTER_PASSED : FILTER_FAILED;
            }

            return filterResults[dictionaryId] == FILTER_PASSED;
        }

        private boolean evaluateFilter(int dictionaryId)
        {
            Slice slice = dictionaryBlock.getSlice(dictionaryId, 0, dictionaryBlock.getSliceLength(dictionaryId));
        ...
        {
            int dictionaryId = adjustValue(value, offsetIndex);
            if (filter != null && !test(dictionaryId)) {
                return false;
            }

            addResult(offsetIndex, dictionaryId);
            return true;
        }
        ...
        public int consumeRepeated(int offsetIndex, long value, int count)
        {
            if (inDictionaryStream == null && (deterministicFilter || filter == null)) {
                ...
                int dictionaryId = toIntExact(value);
                if (filter != null && !test(dictionaryId)) {
                    return 0;
                }
                for (int i = 0; i < count; i++) {
                    addResult(offsetIndex + i, dictionaryId);
                }
                return count;
            }
            int added = 0;
            for (int i = 0; i < count; i++) {
                int dictionaryId = adjustValue(value, offsetIndex + i);
                if (filter != null && !test(dictionaryId)) {
                    continue;
                }
                addResult(offsetIndex + i, dictionaryId);
